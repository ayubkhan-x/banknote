document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!continentsContainer || !subregionsContainer || !clearFilterButton) {
        console.error("Ошибка: Не найдены контейнеры для континентов, суб-регионов или кнопка очистки.");
        return;
    }
    ...
    fetch("get_filters.php")
        ...
        .then(data => {
            ...
            const checkboxes = document.querySelectorAll('.continent-filter, .subregion-filter');
    ...
    function toggleClearButton() {
        const checkedCheckboxes = document.querySelectorAll('.continent-filter:checked, .subregion-filter:checked');
    ...
    document.getElementById('apply-filter').addEventListener('click', function () {
        const continentIds = Array.from(document.querySelectorAll('.continent-filter:checked'))
            .map(checkbox => checkbox.dataset.id);
        const subregionIds = Array.from(document.querySelectorAll('.subregion-filter:checked'))
            .map(checkbox => checkbox.dataset.id);
